...
app = modal.App("flux_kontext_endpoint", image=flux_kontext_endpoint_image)

with flux_kontext_endpoint_image.imports():
...
class FluxService:
    ...
    def inference(self, request: InferenceRequest):
        ...
        # Run integrity check on generated images
        def check_image_integrity(image):
            image_ = np.array(image) / 255.0
            image_ = 2 * image_ - 1
            image_ = torch.from_numpy(image_).to("cuda", dtype=torch.float32).unsqueeze(0).permute(0, 3, 1, 2)
            return not self.integrity_checker.test_image(image_)

        # Filter out any images that fail integrity check
        # images = [img for img in images if check_image_integrity(img)]
        # if not images:
        #     raise ValueError("All generated images were flagged by the integrity checker. Please try a different prompt or input image.")
